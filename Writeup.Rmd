...
As stated in the introduction, our aim is to build a prediction model that predicts the class A,B,C,D,E from the sensor readings. We can start our analysis with a decission tree classification model:
...
The 0.7 accuracy factor obtained is too low for our purposes, so we need to improve the training method. The article cited in the introduction already mentions that due to the inherent noise present in the sensor data, it is worth trying a *random forest* method. We therefore train our model with the following code:
